...
import { BattingInnings, DeliveryOutcome, Howout, Outcome, MatchType, EventType, UnavailableReason }
    from '../../domain';
...
describe('undo', () => {
    ...
    describe('undo event', () => {
        const inningsWithNonDeliveryWicketEvent = {
            ...matches.startedInnings,
            score: 20,
            wickets: 3,
            events: [{
        ...
        const inningsWithBatterUnavailableEvent = {
            ...matches.startedInnings,
            score: 20,
            wickets: 3,
            events: [{
                ...
                batsmanIndex: 5,
                reason: UnavailableReason.Absent,
            }],
            batting: {
                ...matches.startedInnings.batting,
                batters: [
                    ...
                    },
                    {
                        name: 'Batter 6',
                        playerIndex: 5,
                        unavailableReason: UnavailableReason.Absent,
                        innings: {
                            runs: 0,
                            timeIn: 0,
                            ballsFaced: 0,
        ...
        const [updatedInnings, newBatterIndex, newBowlerIndex] = Undo(inningsWithNonDeliveryWicketEvent, 99, 99);
        const [inningsAfterUnavailable] = Undo(inningsWithBatterUnavailableEvent, 99, 99);

        it('should return the passed in values for batsman and bowler index', () => {
            expect(newBatterIndex).toBe(99);
            expect(newBowlerIndex).toBe(99);
        });
        ...
        it('should remove a wicket from the total when event is a non delivery wicket', () => {
            expect(updatedInnings.wickets).toBe(2);
        });

        it('should remove the wicket from the batter when event is a non delivery wicket', () => {
            expect((updatedInnings.batting.batters[4].innings as BattingInnings).wicket).toBeUndefined();
        });

        it('should not remove a wicket from the total when event is a batter unavailable', () => {
            expect(inningsAfterUnavailable.wickets).toBe(3);
        });

        it('should remove the unavailable reason from the batter when event is batter unavailable', () => {
            expect(inningsAfterUnavailable.batting.batters[5].unavailableReason).toBeUndefined();
        });
    });
});
