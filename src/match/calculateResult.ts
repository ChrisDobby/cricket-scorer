...
export default (match: Match): MatchResult | undefined => {
    if (match.innings.length <= 1) { return { result: Result.Abandoned }; }
    if (match.config.inningsPerSide > 1) { return undefined; }

    const battingFirst = match.innings[0].battingTeam;
    const { battingFirstRuns, battingSecondRuns } = match.innings
        .reduce(
            (scoreObj, i) =>
                ({
    ...
    if (battingFirstRuns === battingSecondRuns) { return { result: Result.Tie }; }
    ...
    if (match.config.type === MatchType.Time &&
